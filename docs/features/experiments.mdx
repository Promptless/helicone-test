...
<Info>
  **Who can use this feature**: Anyone on Pro or Enterprise
  [plan](https://www.helicone.ai/pricing), this feature will be charged at cost. This feature now supports an expanded list of LLMs from additional providers (e.g., Google, Mistralai, X-AI, and Meta-Llama) and introduces the new 'OPENROUTER' provider.
</Info>

## What are Experiments?

Helicone's Experiments is designed to help you tune your LLM prompt, test it on production data, and verify your iterations with quantifiable data. In Helicone, you can define and run LLM-as-a-judge or custom evaluators, then compare the results between prompts side-by-side to determine the best one for production. Helicone now supports a variety of LLM providers and models for experimentation, including the new 'OPENROUTER' provider and additional models from Google (gemini-flash-1.5), Mistralai (mistral-7b-instruct), X-AI (grok-beta), and Meta-Llama (llama-3.2-3b-instruct).
...
- **Model update:** LLMs are sensitive to prompt changes. As new models from providers like Google, Mistralai, X-AI, and Meta with advanced capabilities come out, prompts that worked previously can become less effective.
