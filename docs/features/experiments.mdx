...
description: "Experiments is a spreadsheet-like experience designed to tune your LLM prompts for production. It now supports an expanded list of language models from providers such as Google, Mistralai, X-AI, and Meta-Llama. "
...
Helicone's Experiments is designed to help you tune your LLM prompt, test it on production data, and verify your iterations with quantifiable data. In Helicone, you can define and run LLM-as-a-judge or custom evaluators, then compare the results between prompts side-by-side to determine the best one for production. Helicone's Experiments now supports a broad range of models from multiple providers including Google, Mistralai, X-AI, and Meta-Llama.
...
- **Access a broader range of models:** The experiment feature supports providers including Google, Mistralai, X-AI, and Meta-Llama, enabling you to test prompts across a diverse set of AI technologies.


## What is the prompt iteration workflow?

1. Create a new experiment (select from a wide range of models from Google, Mistralai, X-AI, Meta-Llama, and more).
...
Head to the `Experiment` tab. If you have a prompt with Helicone (now includes providers such as Google, Mistralai, X-AI, and Meta-Llama), click on `start from a prompt`, then choose the desired prompt version. Otherwise, click on `start from scratch`.
