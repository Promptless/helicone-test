...
description: "Experiments is a spreadsheet-like experience designed to tune your LLM prompts for production.  It supports a wide range of LLM models from providers like OpenAI, Google, Mistralai, X-AI, and Meta-Llama."
...
Helicone's Experiments is designed to help you tune your LLM prompt, test it on production data, and verify your iterations with quantifiable data. In Helicone, you can define and run LLM-as-a-judge or custom evaluators, then compare the results between prompts side-by-side to determine the best one for production. Helicone's Experiments now supports a broader range of LLM providers including Google, Mistralai, X-AI, and Meta-Llama, offering more flexibility for experimentation.
...
- **Model update:** LLMs are sensitive to prompt changes. As new models from providers such as Google, Mistralai, X-AI, and Meta-Llama come out with advanced capabilities, prompts that worked previously can become less effective.
...
1. Create a new experiment (You can now choose from additional models, including Google, Mistralai, X-AI, and Meta-Llama.)
...
Head to the `Experiment` tab. If you have a prompt with Helicone, click on `start from a prompt`, then choose the desired prompt version. Otherwise, click on `start from scratch`. You can now select from a variety of newly added models from providers such as Google, Mistralai, X-AI, and Meta-Llama.
