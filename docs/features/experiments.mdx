...
description: "Experiments is a spreadsheet-like experience designed to tune your LLM prompts for production. Now supports a broader range of models from Google, Mistralai, X-AI, and Meta-Llama."
...
Helicone's Experiments is designed to help you tune your LLM prompt, test it on production data, and verify your iterations with quantifiable data. In Helicone, you can define and run LLM-as-a-judge or custom evaluators, then compare the results between prompts side-by-side to determine the best one for production. Helicone's Experiments supports a wide range of LLM models from providers including OpenAI, Google, Mistralai, X-AI, and Meta-Llama.
...
- **Broader model selection:** With an expanded set of available models from Google, Mistralai, X-AI, and Meta-Llama, you can tailor prompt experimentation to harness the unique capabilities and strengths of each.
...
1. Create a new experiment (You can now select from a significantly expanded list of models, including providers from Google, Mistralai, X-AI, and Meta-Llama, in addition to GPT-based models from OpenAI.)
...
Head to the `Experiment` tab. If you have a prompt with Helicone, click on `start from a prompt`, then choose the desired prompt version. Otherwise, click on `start from scratch`. You can now select from a wide range of LLM providers (Google, Mistralai, X-AI, Meta-Llama, and OpenAI).
