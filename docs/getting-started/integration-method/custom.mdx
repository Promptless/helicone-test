...
// Hence, the only enforced type is 'model' (Helicone now supports an expanded set of providers, including Google, Mistralai, X-AI, and Meta-Llama. If 'gpt' is in the model name, it is automatically recognized as OpenAI; otherwise, it defaults to OpenRouter), you need still add the respective prompt property for your model.
