...
curl --request POST \
  ...
  --header 'Content-Type: application/json' \
  --header 'Helicone-Auth: Bearer <HELICONE_API_KEY>' \
  --header 'Helicone-Target-Provider: Mistral-AI' \
  --header 'Helicone-Target-Url: https://api.mistralai.com' \
  --data '{
...
<Accordion title="X-AI Grok example">
```bash
curl --request POST \
  --url https://gateway.helicone.ai/v1/chat/completions \
  ...
  --header 'Content-Type: application/json' \
  --header 'Helicone-Auth: Bearer <HELICONE_API_KEY>' \
  --header 'Helicone-Target-Provider: XAI-Grok' \
